...
export default function FilingInformation() {
  ...
  const handleFilingInfo = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (user?.taxReturn?.id) {
      ...
      res = await fetch(
        `http://api.taxguru.skillstorm-congo.com:8080/return/${user.taxReturn.id}?username=${user.username}`,
        {
          method: "PUT",
          headers: {
      ...
      res = await fetch(
        `http://api.taxguru.skillstorm-congo.com:8080/return?username=${user?.username}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",

...
export default function FilingInformation() {
  ...
  const handleFilingInfo = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (user?.taxReturn?.id) {
      ...
      res = await fetch(
        `https://api-taxguru.skillstorm-congo.com/return/${user.taxReturn.id}?username=${user.username}`,
        {
          method: "PUT",
          headers: {
      ...
      res = await fetch(
        `https://api-taxguru.skillstorm-congo.com/return?username=${user?.username}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
